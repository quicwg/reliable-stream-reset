...
RESET_STREAM frame ({{Section 3.2 of RFC9000}}). When resetting a stream without
the intent to deliver any data to the receiver, the sender MAY use either
...
An endpoint responds to a STOP_SENDING frame as described in
{{Section 3.5 of RFC9000}}. If it sends a RESET_STREAM_AT frame in
response, it SHOULD set the Reliable Size to zero, since the peer
has indicated that it does not intend to process further stream data.
